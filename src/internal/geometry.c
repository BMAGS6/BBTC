...
bbtc_ib_geometry_validate_float(const bbtc_ib_geometry_float_t* const geometry)
{
    ...
    if (!isfinite(geometry->initial_behind_projectile_volume_m3) ||
        !isfinite(geometry->bore_cross_sectional_area_m2)        ||
        !isfinite(geometry->projectile_effective_base_area_m2)   ||
        !isfinite(geometry->projectile_travel_to_muzzle_m))
    {
